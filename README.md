...
- **`generated/state/`** - Tracks asset state information such as listing status, warnings, and historical data. Includes `state.json` (per-asset lifecycle state and endpoint-validation results) and `dead_chain_streaks.json` (per-chain consecutive all-endpoints-failed run counts, maintained by `report_dead_chains.mjs`).
