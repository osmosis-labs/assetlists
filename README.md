...
- **`generated/state/`** - Tracks asset state information such as listing status, warnings, and historical data. Includes `state.json` (per-asset lifecycle state and endpoint-validation results), `dead_chain_streaks.json` (per-chain consecutive death-signal run counts), `dead_chain_heights.json` (per-chain last block height and consecutive-stall count, for the frozen-height signal), and `counterparty_client_streaks.json` (per-IBC-triple consecutive non-Active counterparty-client readings from the weekly sweep, maintained by `check_ibc_clients.mjs`). The two `dead_chain_*` files are maintained by `report_dead_chains.mjs`.
...
- Validation results are tracked in `generated/state/state.json`
...
- Validates RPC/REST endpoints for all chains (full validation, batched 10 at a time)
  - Tracks validation results in `generated/state/state.json` for endpoint optimization
...
- **Validate Zone Assets** - Checks `osmosis.zone_assets.json` against the asset schema
- **Validate Zone Chains** - Checks `osmosis.zone_chains.json` against the chain schema (endpoint reachability is checked separately by the daily `validateEndpoints.mjs` run)
